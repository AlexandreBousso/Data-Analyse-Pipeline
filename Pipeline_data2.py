...
def run_full_test():
    ...
    try:
        ...
        df_resultat= (df.pipe(df_info).pipe(check_missing).pipe(convert_dtypes, dtype_map=data_types).pipe(column_rename, mapping=mapping_rename_col, keep_others=True).pipe(df_drop_NAN).pipe(aggregate, grpby_columns=grpby, agg_logic=agg_logic).pipe(col_assign, new_col_name="Prix Moyen Unitaire", arg1="Montant de la vente", arg2="Quantité commandée").pipe(df_info).pipe(saving_file, path=export_path, format="csv"))
